...
@auth
    <!-- Trigger Button -->
    <button
        class="bg-red-600 text-white text-sm hover:bg-red-700 mt-1 px-3 py-1 rounded"
        onclick="document.getElementById('report-modal-{{ $type }}-{{ $id }}').showModal()">
        Report {{ ucfirst($type) }}
    </button>

    <!-- Modal -->
    <dialog id="report-modal-{{ $type }}-{{ $id }}" class="rounded-md w-96 p-4 border border-black bg-gray-200 text-black">
        <form method="POST" action="{{ route('report') }}" class="space-y-3">
            ...
            <textarea name="Reason" rows="4"
                      class="w-full p-2 border rounded bg-gray-200 text-black"
